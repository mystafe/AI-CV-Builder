...
// Minimal client that mirrors backend/routes/api.ts contracts.
// Frontend should import this to avoid drift.
...
export function createApiClient(opts: ApiClientOptions = {}) {
  ...
  // Schemas (should be kept in sync with backend/routes/api.ts)
  const Cv = z.object({
    personal: z.object({
  ...
  return {
    ...
    skillAssessment: {
      generate: (input: {
        ...
        }>("/api/skill-assessment/generate", input),
      grade: (input: {
        sessionId: string
